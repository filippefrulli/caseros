...
export function AdminNewOrderEmail({ orderId, buyerEmail, items, totalAmount, currency, appUrl }: Props) {
  const shortId = orderId.slice(-8).toUpperCase();

  return (
    <Html>
      <Head />
      <Preview>New paid order #{shortId}</Preview>
      <Body style={body}>
        <Container style={container}>
          <Heading style={h1}>caseros</Heading>
          <Heading style={h2}>New order #{shortId}</Heading>
          <Text style={text}>
            A buyer ({buyerEmail}) just placed a paid order.
          </Text>
          ...
          <Section style={{ textAlign: "center", marginTop: "24px" }}>
            <Link href={`${appUrl}/admin/orders`} style={button}>
              View in admin
            </Link>
          </Section>
